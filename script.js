...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('.split-text-reveal').forEach(el => splitObserver.observe(el));

  /* ---- REVEAL LEFT/RIGHT ---- */
  const revealDirObserver = new IntersectionObserver(entries => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        entry.target.classList.add('visible');
        revealDirObserver.unobserve(entry.target);
      }
    });
  }, { threshold: 0.1 });
  document.querySelectorAll('.reveal-left, .reveal-right, .reveal-clip, .line-draw').forEach(el => revealDirObserver.observe(el));
  ...
  if (parallaxImages.length && window.matchMedia('(prefers-reduced-motion: no-preference)').matches) {
    window.addEventListener('scroll', () => {
      parallaxImages.forEach(img => {
        const rect = img.closest('.parallax-img').getBoundingClientRect();
